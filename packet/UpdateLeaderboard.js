    ...
    switch (this.packetLB) {
        ...
        case 49: // FFA-type Packet (List)
            ...
            for (var i = 0; i < lb.length; i++) {
                ...
                bufferSize += item.getName()+item.pID+item.clan_id ? item.getName().length * 8 : 0; // Name
                bufferSize += 2; // Name terminator
            ...
            for (var i = 0; i < lb.length; i++) {
                ...
                // var pID = 0; // Get node id of player's 1st cell
                var clan_id = 0; // Get node id of player's 1st cell
			   ...
			   if (item.cells[0]) {
                ...
                offset += 4;
                 // Set name	
                var name = item.getName()+"i"+item.pID+"i"+item.clan_id;
                if (name) {
